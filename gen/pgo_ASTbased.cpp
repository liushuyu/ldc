...
void CodeGenPGO::emitCounterIncrement(const RootObject *S) const {
  ...
  gIR->ir->CreateCall(GET_INTRINSIC_DECL(instrprof_increment, gIR->ir->getVoidTy()),
...
void CodeGenPGO::valueProfile(uint32_t valueKind, llvm::Instruction *valueSite,
  ...
  if (instrumentValueSites && RegionCounterMap) {
    ...
    gIR->ir->CreateCall(GET_INTRINSIC_DECL(instrprof_value_profile, gIR->ir->getVoidTy()), Args);

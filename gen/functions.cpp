...
void DtoDefineFunction(FuncDeclaration *fd, bool linkageAvailableExternally) {
  ...
  if (f->isDstyleVariadic()) {
    ...
    llvm::CallInst::Create(GET_INTRINSIC_DECL(vastart, LLType::getVoidTy(gIR->context())), llAp, "",
                           gIR->scopebb());
    ...
    // Push cleanup block that calls va_end to match the va_start call.
    {
      ...
      gIR->ir->CreateCall(GET_INTRINSIC_DECL(vaend, LLType::getVoidTy(gIR->context())), llAp);
